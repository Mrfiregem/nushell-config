...
export def "todo edit" [
    ...
    for col in ($todo_record | columns) {
        ...
        match $col {
            'complete' => { if ($todo_record.complete | describe -d).type not-in [bool, nothing] {
                error make {
                    msg: 'Invalid type for complete'
                    label: {span: $todo_record_span, text: 'Complete should be a boolean or null'}
                }
            }}
            'priority' => { if $'($todo_record.priority)' not-in (seq char A Z | append null) {
                error make {
                    msg: 'Invalid type for priority'
                    label: {span: $todo_record_span, text: 'Priority should be a letter [A-Z] or null'}
                }
            }}
            'completion_date' => { match ($todo_record.completion_date | describe -d).type {
                'string' => { if $todo_record.completion_date !~ '^\d{4}-\d{2}-\d{2}$' {
                    error make {
                        msg: 'Invalid type for completion_date'
                        label: {span: $todo_record_span, text: 'Completion date should be formatted as YYYY-MM-DD'}
                    }
                }}
                'date' => { $todo_record.completion_date = $todo_record.completion_date | format date '%F' }
                'nothing' => {}
                _ => { error make {
                    msg: 'Invalid type for completion_date'
                    label: {span: $todo_record_span, text: 'Completion date should be a string, datetime, or null'}
                }}
            }}
            'creation_date' => { match ($todo_record.creation_date | describe -d).type {
                'string' => { if $todo_record.creation_date !~ '^\d{4}-\d{2}-\d{2}$' {
                    error make {
                        msg: 'Invalid type for creation_date'
                        label: {span: $todo_record_span, text: 'Creation date should be formatted as YYYY-MM-DD'}
                    }
                }}
                'date' => { $todo_record.creation_date = $todo_record.creation_date | format date '%F' }
                'nothing' => {}
                _ => { error make {
                    msg: 'Invalid type for creation_date'
                    label: {span: $todo_record_span, text: 'Creation date should be a string, datetime, or null'}
                }}
            }}
            'description' => {
                $todo_record.description = $'($todo_record.description)' | ansi strip | str trim
                if ($todo_record.description | describe -d).type != string {
                    error make {
                        msg: 'Invalid type for description'
                        label: {span: $todo_record_span, text: 'Description should be coersible to a string'}
                    }
                } else if ($todo_record.description == '') {
                    error make {
